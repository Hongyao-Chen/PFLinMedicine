...
class Server(object):
    def __init__(self, args, times):
        ...
        self.best_acc_local = [0 for _ in range(self.num_clients)]
    ...
    def select_clients(self):
        ...
        selected_clients = sorted(selected_clients, key=lambda x: x.id)
    ...
    def aggregate_parameters(self):
        ...
        list_global = self.params_tolist(self.global_model)

        for w, client_model in zip(self.uploaded_weights, self.uploaded_models):
            list_global += self.params_tolist(client_model) * w
        self.list_load_to_params(self.global_model, list_global)
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        for i in range(len(accs)):
            self.best_acc_local[i] = max(self.best_acc_local[i], accs[i])
        print(accs)
        if acc == None:
        ...
        print("Best Local Accurancy:",self.best_acc_local)
